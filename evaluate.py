...
DETECTOR_NAMES = ["confidence", "temp_scaled", "energy", "distance", "mc_dropout", "ood", "mahalanobis"]
...
def run_evaluation(model, train_loader, ood_detector, mahalanobis_params, temperature, shift_type, device):
    ...
    scores, preds, targets = evaluate_temperature_scaled(model, shifted_loader, device, temperature=temperature)
    failures = (preds != targets).astype(int)
...
def run_severity_sweep(model, ood_detector, device):
    """Runs practical monitoring experiment across severity levels 1..5."""
    severity_results = {}
    for shift in SHIFT_TYPES:
        severity_results[shift] = {}
        for sev in range(1, 6):
            loader = get_shifted_dataloader(shift_type=shift, severity=sev)
            probs, targets = get_probabilities_and_targets(model, loader, device)
            preds = probs.argmax(axis=1)
            acc = (preds == targets).mean()

            scores, ood_preds, ood_targets = evaluate_ood(model, ood_detector, loader, device)
            failures = (ood_preds != ood_targets).astype(int)
            auroc = roc_auc_score(failures, scores) if 0 < failures.sum() < len(failures) else float("nan")

            severity_results[shift][sev] = {"accuracy": acc, "ood_auroc": auroc}
...
def results_to_markdown_table(all_results):
    header_names = ["Confidence", "Temp-Scaled", "Energy", "Distance", "MC Dropout", "OOD (SVM)", "Mahalanobis"]
    shift_display = {
        "blur": "Blur",
        "noise": "Noise",
    ...
    lines = [
        "| Shift               | " + " | ".join(header_names) + " |",
        "|---------------------|" + "|".join([":----------:"] * len(header_names)) + "|",
    ]
    for shift_type, results in all_results.items():
        row = [f"{results[d]:.3f}" for d in DETECTOR_NAMES]
        lines.append(f"| {shift_display.get(shift_type, shift_type):<19} | " + " | ".join(row) + " |")
    return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    layer_results = compare_embedding_layers(model, train_loader, val_shifted_loader, device, layers=("layer2", "layer3", "final"))
    print_layer_comparison(layer_results)
    plot_layer_comparison(layer_results, save_path="plots/ocsvm_layer_comparison.png")
    ...
    sweep_results = sweep_ocsvm_hyperparameters(
        model,
        train_loader,
        val_shifted_loader,
        device,
    ...
    plot_hyperparam_heatmap(sweep_results, save_path="plots/ocsvm_hyperparam_sweep.png")
    best_params = sweep_results[0]

    print(
        "Training final OOD detector (One-Class SVM, standardized embeddings) "
        f"with layer={best_layer}, nu={best_params['nu']}, gamma={best_params['gamma']}, "
        f"kernel={best_params['kernel']}..."
    )
    ood_detector = train_ood_detector(
        model,
        train_loader,
        device,
        layer=best_layer,
    ...
    for shift_type in SHIFT_TYPES:
        ...
        all_results[shift_type] = run_evaluation(
            model, train_loader, ood_detector, mahalanobis_params, temperature, shift_type, device
        )

    print_results(all_results)
    plot_auroc_summary(all_results, DETECTOR_NAMES, save_path="plots/auroc_summary.png")

    # Run and plot severity sweep to answer monitoring questions
    print("Running severity sweep across levels 1..5 for practical monitoring evaluation...")
    severity_results = run_severity_sweep(model, ood_detector, device)
    plot_severity_sweep(severity_results, save_path="plots/severity_sweep.png")
    ...
    with open("results/evaluation_results.json", "w") as f:
        json.dump(
            {
                "auroc_by_shift": all_results,
                "severity_sweep": severity_results,
                "temperature": temperature,
                "layer_comparison": layer_results,
                "best_layer": best_layer,
                "hyperparam_sweep": sweep_results,
                "best_ocsvm_params": best_params,
            },
            f,
            indent=2,
        )

    print("Saved results/evaluation_results.json and all summary plots successfully.")
